...
.. function:: run_server(publish_method, host='0.0.0.0', port=5000, shared_dir=Path('shared/'))
    ...
    This function starts the server which receives minds from clients and forward it to ``publish_method`` - a publish function e.g. ``publish_method=print`` then the data will be printed to the screen).
